...
model PulseEmission_RisingEdge
  extends Modelica.Blocks.Interfaces.PartialBooleanSISO_small;
  Modelica.Blocks.MathBoolean.RisingEdge RisingDetection
  ...
  parameter Modelica.SIunits.Time pulseLength "Length of the pulse";
equation
  connect(RisingDetection.y, rSFlipFlop.S)
    annotation (Line(points={{-28.4,0},{-12,0}}, color={255,0,255}));
  ...
  connect(u, RisingDetection.u)
    annotation (Line(points={{-140,0},{-49.2,0}}, color={255,0,255}));
  connect(y, rSFlipFlop.Q)
    annotation (Line(points={{120,0},{11,0},{11,0}}, color={255,0,255}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)));
end PulseEmission_RisingEdge;
