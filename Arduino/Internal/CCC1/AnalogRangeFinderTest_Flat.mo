...
model AnalogRangeFinderTest_Flat
...
public
  Modelica.Electrical.Analog.Ideal.IdealClosingSwitch idealClosingSwitch1(Goff=1e-8)
  ...
  LogicalDelay logicalDelay1(delayTime=30e-6)
    annotation (Placement(transformation(extent={{196,8},{176,28}})));
  ...
  Modelica.Blocks.MathBoolean.FallingEdge falling1
    annotation (Placement(transformation(extent={{62,44},{46,60}})));
  ...
  Modelica.Blocks.MathBoolean.FallingEdge falling2
    annotation (Placement(transformation(extent={{62,22},{46,38}})));
equation
  ...
  connect(pulseEmission_FallingEdge.y, logicalDelay1.u) annotation (Line(points=
         {{198,-24},{204,-24},{208,-24},{208,18},{200,18}}, color={255,0,255}));
  connect(logicalDelay1.y, pulseEmission_FallingEdge1.u)
    annotation (Line(points={{174,18},{164,18},{152,18}}, color={255,0,255}));
  connect(idealClosingSwitch1.p, constantVoltage.p)
    annotation (Line(points={{-6,-20},{-46,-20}}, color={0,0,255}));
  connect(pulseEmission_FallingEdge1.y, idealClosingSwitch1.control)
    annotation (Line(points={{126,18},{126,18},{80,18},{80,-6},{4,-6},{4,-13}},
        color={255,0,255}));
  connect(ArduinoEmitListen.n, SensorGround5V.p)
    annotation (Line(points={{-36,8},{-6,8}}, color={0,0,255}));
  connect(falling1.y,rSFlipFlop. S) annotation (Line(points={{44.4,52},{34,52}},
                           color={255,0,255}));
  connect(idealClosingSwitch1.n, SensorGround5V.n2) annotation (Line(points={{
          14,-20},{34,-20},{50,-20},{50,8},{14,8}}, color={0,0,255}));
  connect(SensorGround5V.n1, resistor.p) annotation (Line(points={{14,13},{52,
          13},{52,14},{64,14},{64,-12}}, color={0,0,255}));
  connect(falling1.u, realToBoolean1.y)
    annotation (Line(points={{65.2,52},{70,52},{73,52}}, color={255,0,255}));
  connect(realToBoolean1.u, voltageSensor.v) annotation (Line(points={{96,52},{
          100,52},{100,50},{100,-22},{100,-24},{98,-24}}, color={0,0,127}));
  connect(rSFlipFlop.Q, SensorGround5V.control)
    annotation (Line(points={{11,52},{4,52},{4,16}}, color={255,0,255}));
  connect(rSFlipFlop.R, falling2.y) annotation (Line(points={{34,40},{40,40},{
          40,30},{44.4,30}}, color={255,0,255}));
  connect(falling2.u, pulseEmission_FallingEdge1.y) annotation (Line(points={{
          65.2,30},{80,30},{80,18},{126,18}}, color={255,0,255}));
...
end AnalogRangeFinderTest_Flat;
